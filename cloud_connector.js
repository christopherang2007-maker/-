/* Supabase 登录与同步层。公开 anon key 可安全放在前端；绝不可放 service_role key。 */
// Supabase 配置由 Vercel 环境变量透过 /api/supabase-config 提供。
...
function cloudPanel(){document.body.insertAdjacentHTML('beforeend',`<div id="cloudLogin" style="position:fixed;inset:0;background:#17223be8;z-index:99;display:grid;place-items:center"><form id="cloudForm" style="width:min(390px,90vw);background:#fff;padding:28px;border-radius:16px;box-shadow:0 12px 44px #0005"><h2 style="margin:0 0 8px">安心托育 · 云端登录</h2><p style="color:#6e7b91;font-size:13px;margin:0 0 18px">登录后，资料会与其他老师即时同步。</p><input required name="email" type="email" placeholder="邮箱" style="width:100%;padding:11px;margin:6px 0;border:1px solid #e5eaf2;border-radius:8px"><input required name="password" type="password" placeholder="密码（至少 6 位）" minlength="6" style="width:100%;padding:11px;margin:6px 0;border:1px solid #e5eaf2;border-radius:8px"><div id="loginError" style="min-height:18px;color:#c33;font-size:12px;margin:5px 0"></div><button style="width:100%;padding:11px;border:0;border-radius:8px;background:#3478f6;color:#fff;font-weight:bold;cursor:pointer">登录</button><button type="button" id="signUp" style="width:100%;padding:10px;border:1px solid #3478f6;border-radius:8px;background:#fff;color:#3478f6;margin-top:8px;cursor:pointer">首次使用：创建老师账号</button></form></div><button id="logout" style="position:fixed;right:18px;bottom:18px;z-index:5;border:0;border-radius:99px;background:#17223b;color:#fff;padding:9px 13px;display:none;cursor:pointer">退出云端账号</button>`);document.getElementById('cloudForm').onsubmit=e=>login(e,false);document.getElementById('signUp').onclick=e=>login(e,true);document.getElementById('logout').onclick=async()=>{await cloud.auth.signOut();location.reload()}}
async function login(e,signup){if(e)e.preventDefault();let f=document.getElementById('cloudForm'),email=f.email.value,password=f.password.value,err=document.getElementById('loginError');err.textContent='';let r=signup?await cloud.auth.signUp({email,password}):await cloud.auth.signInWithPassword({email,password});if(r.error){err.textContent=r.error.message;return}if(signup&&!r.data.session){err.style.color='#078560';err.textContent='账号已创建，请按邮箱验证信后再登录。';return}await connected(r.data.user||r.data.session?.user)}
async function connected(user){cloudUser=user;let p=await cloud.from('profiles').select('role').eq('id',user.id).single();cloudRole=p.data?.role||'teacher';document.getElementById('cloudLogin').remove();document.getElementById('logout').style.display='block';await pullCloud();if(cloudRole!=='admin')document.querySelector('[data-page="admin"]').style.display='none'}
