...
(()=>{
  ...
  function mealNameMarkup(student){
    const diet=String(student?.diet||'').trim();
    return diet?`<span class="special-food-name">${html(student.name)}</span><span class="special-food-detail">（${html(diet)}）</span>`:`<b>${html(student.name)}</b>`;
  }
  ...
  function renderMealPlanning(){
    ...
    $('schoolPackMeals').innerHTML=schoolMeals.length?schoolMeals.map(({student,decision})=>`<div class="school-pack-item ${decision.needed?'':'cancelled'}"><div>${mealNameMarkup(student)} · ${html(student.grade)} · ${html(student.meal)}</div>${decision.needed?'留校，需要带饭':`不用包饭：${html(decision.reason||'特殊情况')}`}</div>`).join(''):'<p class="muted">这个星期没有设置“留校＋需要带饭”的学生。</p>';
